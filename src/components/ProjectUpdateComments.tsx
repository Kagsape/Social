...
const ProjectUpdateComments = ({ updateId }: ProjectUpdateCommentsProps) => {
            ...
            <div key={comment.id} className="flex gap-3 group">
              ...
              <div className="flex-1 bg-slate-50 dark:bg-slate-800/50 rounded-xl px-3 py-2">
                <div className="flex items-center justify-between">
                  <Link to={`/profile/${comment.user_id}`} className="text-[10px] font-bold hover:underline">
                    {comment.users?.name}
                  </Link>
      ...
      {user && (
        <form onSubmit={handleSubmit} className="flex gap-2 items-center">
          ...
          <Input
            ...
            className="h-7 text-[10px] rounded-full bg-slate-50 dark:bg-slate-800/50 border-none focus-visible:ring-1"
          />
          <Button 
            type="submit" 
            size="icon" 
